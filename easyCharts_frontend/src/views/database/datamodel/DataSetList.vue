<template>
  <div class="page">
        ...
        <div  style="height: calc(100% - 85px);">
          <vxe-table
              ...
              <vxe-column
                ...
                sortable
                title="模型名称">
                <template #default="scope"><el-link  type="primary" :underline="false" @click="view(scope.row.id)">{{scope.row.name}}</el-link>
                </template>
              </vxe-column>
              <vxe-column
                  ...
                  <template  #default="scope">
                    <p v-if="scope.row.data_source_id === 'master'">
                      本地数据库
                    </p>
                    <p v-else>
              ...
              <vxe-column
                ...
                <template  #default="scope">
                  ...
                  <el-button link size="small" @click="getDbInterface(scope.row.id)">获取数据接口</el-button>
    ...
    <el-dialog title="数据接口" v-model="dialogInterfaceVisible">
...
<script>
  ...
  import {ElMessage, ElMessageBox,ElInput, ElButton,ElForm,ElFormItem,ElLink,ElDivider,ElTableColumn,ElTable,ElDialog} from 'element-plus'
  ...
  export default {
    ...
    methods: {
      ...
      edit (id) {
        ...
        this.$router.push({path: `/DataSetForm`, query: {id: id, title: '数据模型配置'}})
      },
      // 查看
      view (id) {
        id = id || this.$refs.dataSetTable.getCheckboxRecords().map(item => {
        ...
        this.$router.push({path: `/DataSetForm`, query: {method: 'view', id: id, title: '数据模型配置'}})
      },
      onCopy (e) {
      ...
      del(id) {
        ...
        ElMessageBox.confirm(
          ...
          '提示',
          {
            ...
            this.loading = true
            dataSetService.delete(ids).then(res => {
              ElMessage({
                type: 'success',
                message: res
              })
